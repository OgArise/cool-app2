...
if config.GCP_SERVICE_ACCOUNT_JSON_STR:
    try:
        ...
        print(f"ERROR: Could not parse GCP_SERVICE_ACCOUNT_JSON_STR environment variable: {e}")
else:
    print("Warning: GCP_SERVICE_ACCOUNT_JSON_STR environment variable not set.")

# Sheet Names (must match your tabs)
...
def _get_gsheet_service():
    """Authenticates and builds the Google Sheets service object."""
    creds = None
    ...
    try:
        ...
        service = build('sheets', 'v4', credentials=creds, cache_discovery=False) # Disable cache for serverless environments
...
def _append_to_gsheet(service, sheet_name: str, values: List[List[Any]]):
    ...
    if not service or not SHEET_ID:
        print(f"Skipping append to {sheet_name}: Missing service or Sheet ID.")
        return False
    try:
        body = {'values': values}
...
def _save_analysis_to_gsheet(run_results: Dict):
    ...
    if not service:
        print("Aborting save to Google Sheets due to authentication failure.")
        return

    run_timestamp = datetime.now().isoformat() # Use timestamp as a unique run identifier
    ...
    _append_to_gsheet(service, SHEET_NAME_RUNS, [run_summary_row]) # Append wants a list of lists
    ...
    if relationships:
        rel_rows = [
             # Corrected field names based on potential output structure
...
# --- Main Orchestration Logic (remains largely the same, just calls gsheet save) ---
def run_analysis(initial_query: str,
    ...
    try:
        ...
        elif not config.LLM_PROVIDER: print("Warning: LLM Provider not configured, skipping global NLP extraction.")
        else: print("No global results to process for NLP.")
        ...
        step2_start = time.time(); translated_keywords = []; specific_query = initial_query
        ...
        else: print("Warning: LLM Provider not configured, skipping keyword translation.")
        ...
        elif not config.LLM_PROVIDER: print("Warning: LLM Provider not configured, skipping specific NLP extraction.")
        else: print("No specific results to process for NLP.")
        ...
        if kg_driver_available:
            ...
            unique_relationships = list({f"{r.get('entity1', '')}_{r.get('relationship_type', '')}_{r.get('entity2', '')}".lower(): r for r in results["final_extracted_data"]["relationships"] if r.get('entity1') and r.get('relationship_type') and r.get('entity2')}.values()) # Corrected keys to entity1/entity2
            ...
            if not any([unique_entities, unique_risks, unique_relationships]):
                 print("No unique data extracted to update knowledge graph."); kg_status_message = "skipped_no_data"
            else:
        ...
        else: print("Skipping KG update because driver is not available.")
        ...
        if config.GOOGLE_SHEET_ID and config.GCP_SERVICE_ACCOUNT_JSON_STR: # Check if configured
            _save_analysis_to_gsheet(results)
        else:
             print("Skipping save to Google Sheets: Configuration missing.")
        # --- End Save ---
