...
try:
    from google.oauth2 import service_account; from googleapiclient.discovery import build; import os; google_sheets_available = True
...
import nlp_processor # Uses dynamic LLM client
import knowledge_graph
import config

# --- Google Sheets Config & Functions ---
# ... (Keep _get_gsheet_service, _append_to_gsheet, _save_analysis_to_gsheet functions exactly as provided in Response #57) ...
# Make sure _save_analysis_to_gsheet includes the column for analysis_summary in the Runs sheet data row.
# Example addition to _save_analysis_to_gsheet's run_summary_row:
# run_summary_row = [ run_timestamp_iso, run_results.get('query'), ..., run_results.get('error'), run_results.get('analysis_summary', '')[:500] ]
SCOPES = ['https://www.googleapis.com/auth/spreadsheets']; SHEET_ID = config.GOOGLE_SHEET_ID; SERVICE_ACCOUNT_INFO = None; GCP_JSON_STR = config.GCP_SERVICE_ACCOUNT_JSON_STR
...
SHEET_NAME_RUNS = 'Runs'; SHEET_NAME_ENTITIES = 'Entities'; SHEET_NAME_RISKS = 'Risks'; SHEET_NAME_RELATIONSHIPS = 'Relationships'; SHEET_NAME_EXPOSURES = 'Supply Chain Exposures'
gsheet_service = None
def _get_gsheet_service(): global gsheet_service; if not google_sheets_available: return None; if gsheet_service is None and SERVICE_ACCOUNT_INFO: try: creds = service_account.Credentials.from_service_account_info(SERVICE_ACCOUNT_INFO, scopes=SCOPES); gsheet_service = build('sheets', 'v4', credentials=creds, cache_discovery=False); print("GSheet service authenticated.") except Exception as e: print(f"ERROR auth GSheets: {e}"); gsheet_service = None; return gsheet_service; return gsheet_service
def _append_to_gsheet(service, sheet_name: str, values: List[List[Any]]): if not service or not SHEET_ID: return False; if not values: return True; try: body = {'values': values}; result = service.spreadsheets().values().append(spreadsheetId=SHEET_ID, range=f"{sheet_name}!A1", valueInputOption='USER_ENTERED', insertDataOption='INSERT_ROWS', body=body).execute(); updated_rows = result.get('updates', {}).get('updatedRows', 0); if updated_rows > 0: print(f"Appended {updated_rows} rows to sheet '{sheet_name}'."); return True; except Exception as e: print(f"ERROR appending to GSheet '{sheet_name}': {e}"); return False
def _save_analysis_to_gsheet(run_results: Dict): print("Saving to GSheets..."); service = _get_gsheet_service(); if not service: print("Abort save: GSheet service unavailable."); return; run_timestamp_iso = "Timestamp Error"; try: dt_utc = datetime.now(timezone.utc); if zoneinfo_available: mountain_tz = ZoneInfo("America/Denver"); dt_mt = dt_utc.astimezone(mountain_tz); run_timestamp_iso = dt_mt.isoformat(timespec='seconds'); print(f"MT timestamp: {run_timestamp_iso}") else: run_timestamp_iso = dt_utc.isoformat(timespec='seconds'); print(f"UTC timestamp: {run_timestamp_iso}") except Exception as e: print(f"ERROR timestamp: {e}"); run_timestamp_iso = datetime.now().isoformat(timespec='seconds') + "_Error"; run_summary_row = [ run_timestamp_iso, run_results.get('query'), run_results.get('run_duration_seconds'), run_results.get('kg_update_status'), run_results.get('error'), run_results.get('analysis_summary', '')[:500] ]; _append_to_gsheet(service, SHEET_NAME_RUNS, [run_summary_row]); extracted = run_results.get("final_extracted_data", {}); entities = extracted.get("entities", []); risks = extracted.get("risks", []); relationships = extracted.get("relationships", []); if entities: entity_rows = [[run_timestamp_iso, e.get('name'), e.get('type'), json.dumps(e.get('mentions', []))] for e in entities if isinstance(e, dict) and e.get('name')]; _append_to_gsheet(service, SHEET_NAME_ENTITIES, entity_rows); if risks: risk_rows = [[run_timestamp_iso, r.get('description'), r.get('severity'), json.dumps(r.get('related_entities', [])), json.dumps(r.get('source_urls', []))] for r in risks if isinstance(r, dict) and r.get('description')]; _append_to_gsheet(service, SHEET_NAME_RISKS, risk_rows); if relationships: rel_rows = [[run_timestamp_iso, rel.get('entity1'), rel.get('relationship_type'), rel.get('entity2'), json.dumps(rel.get('context_urls', []))] for rel in relationships if isinstance(rel, dict) and rel.get('entity1') and rel.get('relationship_type') and rel.get('entity2')]; _append_to_gsheet(service, SHEET_NAME_RELATIONSHIPS, rel_rows); exposures = run_results.get("supply_chain_exposures", []); if exposures: exposure_rows = [ [run_timestamp_iso, exp.get('problematic_sub_affiliate'), exp.get('relationship_type'), exp.get('parent_company'), exp.get('reason_involved'), exp.get('risk_source_url'), exp.get('ownership_source_url') ] for exp in exposures if isinstance(exp, dict) and exp.get('problematic_sub_affiliate')]; _append_to_gsheet(service, SHEET_NAME_EXPOSURES, exposure_rows); print("GSheets save process finished.")
...
def run_analysis(initial_query: str,
                 ...
                 specific_country_code: str = 'us', # Default to US if global selected in UI
    ...
    if not llm_provider or not llm_model:
        results["error"] = "Missing LLM config."; print(f"ERROR: {results['error']}")
    ...
    try:
        ...
        # Search logic with fallback (as before)
        ...
        # Call extraction WITHOUT China focus
        ...
        # Search logic using country code (as corrected before)
        ...
        # ===> Call extraction WITH China focus flag <===
        is_china_focused = (specific_country_code == 'cn')
        specific_extracted_data = nlp_processor.extract_data_from_results(specific_search_results, specific_search_context, llm_provider, llm_model, focus_on_china=is_china_focused)
        print(f"Extracted {len(specific_extracted_data.get('entities',[]))} entities, {len(specific_extracted_data.get('risks',[]))} risks specifically (China focus: {is_china_focused}).")
        ...
        # ... (Keep logic as before, including the OWNERSHIP_CHECK_DELAY = 4.0) ...
