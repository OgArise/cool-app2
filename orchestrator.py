...
try:
    ...
    print("Warning: Google API libs not installed. Saving to GSheets disabled.")
...
def _append_to_gsheet(service, sheet_name: str, values: List[List[Any]]):
    # ... (keep function as before - it handles service=None check) ...
...
def _save_analysis_to_gsheet(run_results: Dict):
    # ... (keep function as before - handles saving all sections including exposures) ...
    ...
    # Save Run Summary
    run_summary_row = [ run_timestamp, run_results.get('query'), run_results.get('run_duration_seconds'), run_results.get('kg_update_status'), run_results.get('error'), run_results.get('analysis_summary', '')[:500] ] # Add Summary (truncated)
    ...
    # Save Supply Chain Exposures
    exposures = run_results.get("supply_chain_exposures", [])
    if exposures: exposure_rows = [[ run_timestamp, exp.get('problematic_sub_affiliate'), exp.get('relationship_type'), exp.get('parent_company'), exp.get('reason_involved'), exp.get('risk_source_url'), exp.get('ownership_source_url') ] for exp in exposures if isinstance(exp, dict) and exp.get('problematic_sub_affiliate')]; _append_to_gsheet(service, SHEET_NAME_EXPOSURES, exposure_rows)
...
def run_analysis(initial_query: str,
                 ...
                 specific_search_context: str = "search for specific company examples and regulatory actions", # Updated Default
    ...
    # Basic check if LLM params are missing
    if not llm_provider or not llm_model:
        results["error"] = "Missing LLM configuration."; print(f"ERROR: {results['error']}")
        # ... (save partial results and return logic) ...
    ...
    try:
        ...
        # Use Google Official Search if available, otherwise fallback to SerpApi Google
        ...
        elif config.SERPAPI_KEY and search_engines.serpapi_available: # Check if SerpApi available
             print("Using SerpApi Google for global search..."); global_raw_results = search_engines.search_via_serpapi(initial_query, engine='google', country_code='us', lang_code='en', num=max_global_results); global_search_source = 'serpapi_google'
        else: print("Warning: No suitable global search engine configured/available.")
        ...
        step3_start = time.time(); specific_raw_results = []; specific_search_source = 'unknown_specific'
        # Determine engine based on code. Use Google API/SerpApi if not 'cn'
        ...
        # ===> ADDED DELAY TO MITIGATE RATE LIMITING <===
        OWNERSHIP_CHECK_DELAY = 2.0 # Seconds between LLM calls for ownership check (adjust as needed)
        # ===> END ADDITION <===
        ...
        for parent_name, risk_info in high_risk_parent_entities.items():
            ...
            for related_name in potential_related_names:
                 ...
                 # ===> ADDED DELAY <===
                 print(f"Pausing {OWNERSHIP_CHECK_DELAY}s before next ownership check to avoid rate limits...")
                 time.sleep(OWNERSHIP_CHECK_DELAY)
                 # ===> END DELAY <===
        ...
        if config.GOOGLE_SHEET_ID and SERVICE_ACCOUNT_INFO: _save_analysis_to_gsheet(results)
        else: print("Skipping save to Google Sheets: Config missing/invalid.")
